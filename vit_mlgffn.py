...
class WindowAttention(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        debug_print(f"WindowAttention input shape: {x.shape}")

        # Unfold input
        x_unfolded = F.unfold(x, kernel_size=self.window_size, stride=self.window_size)
        debug_print(f"After unfolding: {x_unfolded.shape}")
        ...
        debug_print(f"Q, K, V shapes: {q.shape}, {k.shape}, {v.shape}")
        ...
        debug_print(f"Reshaped Q, K, V shapes: {q.shape}, {k.shape}, {v.shape}")

        attn = (q @ k) * self.scale
        debug_print(f"Attention shape before softmax: {attn.shape}")
        
        attn = attn.softmax(dim=-1)
        debug_print(f"Attention shape after softmax: {attn.shape}")

        # Store attention weights for visualization
        self.attention_weights = attn.detach()

        x = (attn @ v).transpose(-1, -2).contiguous()
        debug_print(f"Shape after attention application: {x.shape}")
        
        x = x.view(-1, C, self.window_size[0], self.window_size[1])
        debug_print(f"Shape after reshaping: {x.shape}")
        
        x = self.proj(x)
        debug_print(f"Shape after projection: {x.shape}")

        # Fold output
        x = x.view(B, -1, C * self.window_size[0] * self.window_size[1]).transpose(1, 2)
        debug_print(f"Shape before folding: {x.shape}")
        
        x = F.fold(x, output_size=(H, W), kernel_size=self.window_size, stride=self.window_size)
        debug_print(f"WindowAttention output shape: {x.shape}")
...
class GlobalAttention(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        debug_print(f"GlobalAttention input shape: {x.shape}")
        ...
        debug_print(f"GlobalAttention output shape: {x.shape}")
        return x
...
class SpatialAwareSelfAttention(nn.Module):
    ...
    def forward(self, x):
        debug_print(f"SpatialAwareSelfAttention input shape: {x.shape}")
        
        # Window attention
        x_windows = self.window_attn(x)
        debug_print(f"SpatialAwareSelfAttention after window attention: {x_windows.shape}")
        
        # Global attention
        x_global = self.global_attn(x)
        debug_print(f"SpatialAwareSelfAttention after global attention: {x_global.shape}")
        
        # Combine local and global attention
        x = x_windows + x_global
        
        debug_print(f"SpatialAwareSelfAttention output shape: {x.shape}")
        return x
...
class HSCATB(nn.Module):
    ...
    def forward(self, x):
        debug_print(f"HSCATB input shape: {x.shape}")
        x = x + self.attn(self.norm1(x))
        debug_print(f"HSCATB after spatial attention: {x.shape}")
        x = x + self.casa(self.norm2(x))
        debug_print(f"HSCATB after channel attention: {x.shape}")
